...
const boardMaker = (boardObject) => {
  const domString = `<div class="card board" style="width: 18rem;" id="${boardObject.uid}">
                     <img src="${boardObject.imageUrl}" class="card-img-top" style="width:18rem; height:12rem;" alt="${boardObject.name}">
                  <div class="card-body">
                    <h5 class="card-title">${boardObject.name}</h5>
                    <a href='#' id="${boardObject.uid}" class="btn btn-info view-pins"> View Pins</a>
                    <a href='#' id="${boardObject.uid}"
                        class="update-board btn btn-success"><i class="far fa-edit"></i> Update Board</a>
